...
export class PublisherDetailComponent {
    ...
    updateQueryParams(first: boolean = false){
        ...
        // reload series after
        const slug = this.route.snapshot.paramMap.get('slug');
        if(first) this.loadPublisher(slug);
        else this.loadWorks();
    }
    ...
    loadWorks(){
        ...
        this.api.request<any>(HttpMethod.POST, `publisher/publications/${this.publisher.id}?user_lang=${lang}`, {order: this.selectedOrder.value, limit: 12, offset, search: this.search, user_lang: lang}).subscribe((res:any)=>{
            this.maxPages = res.max;

            // check if aliases were matched in backend
            for(const item of res.publications){
                console.log(item)
    ...
    relationClick(slug:string){
        this.router.navigate(['publisher',slug]);
    }
