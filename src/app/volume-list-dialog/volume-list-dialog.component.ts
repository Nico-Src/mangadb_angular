...
@Component({
    ...
    templateUrl: './volume-list-dialog.component.html',
    styleUrl: './volume-list-dialog.component.less',
    providers: [tuiDateFormatProvider({mode: 'YMD', separator: '/'})],
})
export class VolumeListDialog {
    ...
    // load user rating
    loadLists(){
        this.api.request<any>(HttpMethod.GET, `lists/type/volume/${this.volume.id}`, {}).subscribe((res:any)=>{
            this.lists = res;
    ...
    async addToList(){
        ...
        // send api request
        this.api.request<any>(HttpMethod.POST, `lists/add/volume`, {item_id: this.volume.id, list_id: this.selectedList.id}, 'text').subscribe(async (res:string)=>{
            const msg = await getTranslation(this.translate, `list-dialog.add-volume-success`);
            ...
            if(err.status === 409){
                const msg = await getTranslation(this.translate, `list-dialog.already-in-list-volume`);
                errorAlert(this.alerts, msg, undefined, this.translate);
            } else {
